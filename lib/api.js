...
const datenDirectory = join(process.cwd(), '_rechtliches')

export function getDatenSlugs() {
  return fs.readdirSync(datenDirectory)
}

export function getDatenBySlug(slug, fields = []) {
  const realSlug = slug.replace(/\.md$/, '')
  const fullPath = join(datenDirectory, `${realSlug}.md`)
  const fileContents = fs.readFileSync(fullPath, 'utf8')
  const { data, content } = matter(fileContents)
...
export function getAllDaten(fields = []) {
  const slugs = getDatenSlugs()
  const daten = slugs
    .map((slug) => getDatenBySlug(slug, fields))
    // sort posts by date in descending order
    // .sort((daten1, daten2) => (daten1.date > daten2.date ? -1 : 1))
  return daten
}